...
class ObjectClassifier(nn.Module):
    ...
    def __init__(
        ...
        embed_vecs = obj_edge_vectors(obj_classes[1:], wv_type='glove.6B', wv_dir=word_vec_dir, wv_dim=word_vec_dim)
    ...
    def clean_class(self, entry, b, class_idx):
        ...
        for i in range(b):
            ...
            if new_scores.shape[0] > 0:
                ...
                new_labels = torch.tensor([], dtype=torch.long).to(self.device)
    ...
    def forward(self, entry):
        ...
        if self.mode  == 'predcls':
            ...
            if self.training:
                ...
                HUMAN_IDX = torch.zeros([b, 1], dtype=torch.int64).to(self.device)
            ...
            if self.training:
                ...
                pair_rois = torch.cat((entry['boxes'][pair[:, 0], 1:], entry['boxes'][pair[:, 1], 1:]),
                                      1).data.cpu().numpy()
                entry['spatial_masks'] = torch.tensor(draw_union_boxes(pair_rois, 27) - 0.5).to(self.device)
